...
# 追加仕様：足の動きを強化（サイドステップ ＆ 下半身ポーズ）

現状、上半身ばかり使って足が暇になっています。
全身運動にするため、以下の2つの大きな変更を行ってください。

## 1. 壁の出現位置ランダム化（サイドステップ強要）
壁の穴（および壁の中心点）が、常に画面中央（X=0.5）に来るのではなく、**「左寄り・中央・右寄り」の3レーン**からランダムで選ばれるようにしてください。

- **仕様:**
  - 壁生成時、`offsetX` をランダムに決定する（例: 画面幅の 30%, 50%, 70% の位置）。
  - プレイヤーは、その位置まで移動（サイドステップ）してからポーズをとる必要がある。
  - **重要:** 判定ロジックも、画面中央ではなく「壁の `offsetX`」を基準に、プレイヤーの重心（Hipsの中心）が合っているかを最初にチェックする。位置がズレていたら、ポーズが合っていても即アウト（MISS）。

- **視覚ガイド:**
  - プレイヤーがどこに立てばいいかわかるように、床（画面下部）に「ここに立て！（STAND HERE）」という矢印や枠線マーカーを表示してください。

## 2. 下半身強化ポーズの追加
足を使わせる新しいポーズを2つ追加してください。

### A. 「SUMO（四股）」
- **壁の形:** 力士が四股を踏んでいるような、横に広くて背が低い穴。
- **判定条件:**
  - 両足首(Ankles)のX座標の距離が、肩幅の2倍以上開いていること。
  - 腰(Hips)のY座標が、通常時より明らかに下がっていること（深いスクワット）。
  - 手は膝の上あたりにあるか、腰に構えている。

### B. 「KICK（サイドキック）」
- **壁の形:** Y字バランスのように、片足だけ横に飛び出している穴。
- **判定条件:**
  - 左右どちらかの足首(Ankle)のY座標が、腰(Hip)の高さ付近まで上がっていること。
  - 軸足は地面についていること。
  - （転倒防止のため、判定は少し甘めに設定してください）

## 実装上の注意
- サイドステップが入るため、カメラの画角からフレームアウトしないよう、「もっと下がって！」等の警告が必要ならUIに出してください。
- 判定時は「位置合わせ（X軸）」と「ポーズ（形状）」の両方をクリアする必要があります。